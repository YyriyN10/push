...
export default function Edit({ attributes, setAttributes, clientId }) {
	...
	const {itemQuestion, answer} = attributes;
	...
	const migrateLegacyToInnerBlocks = () => {
		if ( !answer ) return;
		const blocks = rawHandler( { HTML: answer } ) || [];
		replaceInnerBlocks( clientId, blocks, false );
		setAttributes( { answer: '' } ); // очистити легасі, щоб контент не дублювався
		setMigrated( true );
	};
		...
		<>
			...
			{!!answer && !hasInner && (
				<Notice status="info" isDismissible={ false }>
					{ __( 'Виявлено старий текст відповіді. Конвертувати у параграфи/списки?', 'yuna' ) }{' '}
					<Button variant="primary" onClick={ migrateLegacyToInnerBlocks }>
						{ __( 'Конвертувати', 'yuna' ) }
					</Button>
				</Notice>
			)}
			<div { ...blockProps } >
				...
				<div className="item-answer">
					<InnerBlocks
						...
						template={ [
							[ 'core/paragraph', { placeholder: __( 'Текст відповіді…', 'yuna' ) } ],
						] }
						templateLock={ false }
			...
			{ migrated && (
				...
				>
					{ __( 'Готово! Текст сконвертовано у внутрішні блоки.', 'yuna' ) }
				</Notice>
			) }
		</>
	);
